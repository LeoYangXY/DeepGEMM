...
// ---------------------------------------------------------------------------
//  SM120 (Blackwell consumer, RTX 5050) raw FP8 GEMM launcher.
//  Mirrors `SM90FP8Gemm1D1DRuntime` structurally; differences:
//    * the JIT kernel is `sm120_fp8_gemm_1d1d_impl<...>` (no SF TMA)
//    * cluster_size is always 1 (multicast disabled on this arch)
//    * D is FP32; math is unscaled A_fp8 @ B_fp8 (same as cuBLASLt raw FP8)
// ---------------------------------------------------------------------------
...
static void sm120_fp8_gemm_1d1d(const torch::Tensor& a,
    ...
    // Sanity: SM120 kernel needs 128B TMA swizzle (ldmatrix.x4 path)
    DG_HOST_ASSERT(config.storage_config.swizzle_a_mode == 128);
    DG_HOST_ASSERT(config.storage_config.swizzle_b_mode == 128);
