 ...
 *
 * Compile (standalone): nvcc -arch=sm_120 -O3 -lcuda -o hgemm_tma hgemm_tma_warp_spec.cu
 * Python:  python origin_cuda_kernel/sgemm/hgemm_tma_warp_spec.py
 * =============================================================================
 */
...
// =============================================================================
// Torch bindings (compiled when loaded via torch.utils.cpp_extension)
// =============================================================================
...
torch::Tensor torch_hgemm_tma_warp_spec(torch::Tensor A, torch::Tensor B) {
    ...
    TORCH_CHECK(B.size(1) == K, "A.shape[1] must equal B.shape[1] (K)");
    TORCH_CHECK(M % BM == 0 && N % BN == 0 && K % BK == 0,
                "M/N/K must be multiples of BM/BN/BK");

    auto C = torch::zeros({M, N}, A.options());
    CU_CHECK(cuInit(0));

    auto* dA = reinterpret_cast<const half*>(A.data_ptr<at::Half>());
    auto* dB = reinterpret_cast<const half*>(B.data_ptr<at::Half>());
    auto* dC = reinterpret_cast<half*>(C.data_ptr<at::Half>());
    CUtensorMap tA = make_tma(dA, M, K, BM, BK);
    CUtensorMap tB = make_tma(dB, N, K, BN, BK);

    dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);
    dim3 block(THREADS_PER_CTA);
    int smem = sizeof(Smem);
    CUDA_CHECK(cudaFuncSetAttribute(
        hgemm_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    hgemm_kernel<<<grid, block, smem>>>(tA, tB, dC, M, N, K);
    return C;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    TORCH_BINDING_COMMON_EXTENSION(torch_hgemm_tma_warp_spec)
}
